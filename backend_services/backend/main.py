...
def verify_hmac_signature(signed_data: str) -> str:
    """
    Verifies the integrity of the cookie signed by auth.js.
    Matches the signData logic: data + "." + signatureHex
    """
    try:
        if "." not in signed_data:
            raise ValueError("Invalid format")
            
        data, received_signature = signed_data.rsplit('.', 1)
        
        # Recreate the HMAC-SHA256 signature using the shared secret
...
@app.get("/auth/api/form-links")
async def get_form_links(request: Request, email: Optional[str] = None):
    # 1. Retrieve the session cookie created by AuthManager.saveSession()
    session_cookie = request.cookies.get("transfit_session")
    
    print(f"Received request for form links with email={email}, session_cookie={'present' if session_cookie else 'missing'}")
    if not session_cookie:
        raise HTTPException(status_code=401, detail="No session found")

    # 2. Verify that the cookie hasn't been tampered with
    # This proves the data was generated by our AuthManager
    verified_payload = verify_hmac_signature(session_cookie)
    
    # 3. Load clinic data
    # Path adjusted for your directory structure (/Transfit/backend)
    data_path = os.path.join(os.path.dirname(__file__), "form_links.json")
    with open(data_path, "r") as f:
        links_db = json.load(f)
    print(f"Loaded form links for users: {list(links_db.get('users', {}).keys())}")

    # 4. Identity Check
    # Even if the email is passed as a query, the verified_payload 
    # ensures a session actually exists for the user
    user_key = email.strip().lower() if email else "default"
    return links_db.get("users", {}).get(user_key, links_db.get("default"))

# def get_form_links_first(email: Optional[str] = None):
